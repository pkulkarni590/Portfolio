        ...
        <div className="grid gap-6">
          {DATA.projects.map((project, i) => (
            <motion.div
              ...
              className="group p-5 rounded-xl border border-border hover:border-primary/50 bg-white hover:shadow-md transition-all"
            >
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {project.tags.map(tag => (
                  <span key={tag} className="px-2 py-1 bg-primary/10 text-primary text-xs rounded-md font-semibold">{tag}</span>
                ))}
              </div>
            </motion.div>
          ))}
